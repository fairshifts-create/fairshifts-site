...
### Assets
- `images/` — filenames are lowercase (`before.png`, `after.png`, `validation.png`, `logo-light-mode.png`/`.webp`, `logo-dark-mode.png`/`.webp`, `favicon-16/32/48/192.png`, `apple-touch-icon.png`, `og-image.jpg`) and must stay that way: local Windows dev is case-insensitive but Vercel's Linux servers are not, so a case mismatch works locally and 404s in production.
...
- The brand mark (`.brand` in the header/footer) is two `<picture>` elements, not text — `brand-logo-light`/`brand-logo-dark` swap via the same `[data-theme="dark"]` mechanism as everything else. Regenerate both PNG+WebP pairs together if the logo ever changes; don't edit one format without the other.
